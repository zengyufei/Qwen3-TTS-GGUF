...
from transformers.masking_utils import create_causal_mask # Check imports path
...
class MimiConv1d(nn.Module):
    ...
    def _get_extra_padding_for_conv1d(
        ...
        # Dynamo Friendly Integer implementation:
        num = length - self.kernel_size + self.padding_total
        ceil_div = (num + self.stride - 1) // self.stride
        n_frames_ceil = ceil_div + 1
        n_frames = n_frames_ceil - 1
...
class MimiModel(MimiPreTrainedModel):
    ...
    def _encode_frame(
        ...
        if return_dict:
            ...
            embeddings = encoder_outputs.transpose(1, 2)
