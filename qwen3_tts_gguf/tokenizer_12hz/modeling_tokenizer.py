...
from .configuration_tokenizer import (
...
class Qwen3TTSTokenizerV2CausalConvNet(nn.Module):
    ...
    def _get_extra_padding_for_conv1d(self, hidden_state: torch.Tensor) -> int:
        ...
        # Dynamo-friendly integer arithmetic replacement for math.ceil
        # Original: n_frames = (length - self.kernel_size + self.padding) / self.stride + 1
        # wanted: match.ceil(n_frames)
        # Logic: ceil(x/y) = (x + y - 1) // y for integers
        
        num = length - self.kernel_size + self.padding
        # Part encoded by n_frames before the +1: `num / self.stride`
        # We need ceil(num / self.stride)
