...
def main():
    # 1. 配置路径 - Base 模型
    MODEL_DIR = r'./Qwen3-TTS-12Hz-1.7B-Base'
    OUTPUT_DIR = r'./model-base'
    ONNX_PATH = os.path.join(OUTPUT_DIR, 'qwen3_tts_encoder.onnx')

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # 路径处理
    logger.info(f"正在加载 Base 模型: {MODEL_DIR}")
    ...
    try:
        ...
        logger.info("✅ Base Encoder ONNX 导出成功到 model-base 文件夹！")

    except Exception as e:
        logger.error(f"❌ Base Encoder ONNX 导出失败: {e}")
        import traceback
        traceback.print_exc()
        return
